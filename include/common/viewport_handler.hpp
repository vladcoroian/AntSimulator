...
struct ViewportHandler {
  struct State {
    ...
    void updateMousePosition(sf::Vector2f new_position) {
      ...
      const sf::Vector2f pos(static_cast<float>(new_position.x),
                             static_cast<float>(new_position.y));
  ...
  ViewportHandler(sf::Vector2f size) : state(size) { state.updateState(); }
  ...
  void click() { state.clicking = true; }

  void unclick() { state.clicking = false; }
