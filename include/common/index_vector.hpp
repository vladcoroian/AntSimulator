...
struct Vector : public GenericProvider {
 ...
 public:
  ...
  [[nodiscard]] ID getID(uint64_t i) const;
  // Returns the data emplacement of an ID
  [[nodiscard]] uint64_t getDataID(ID id) const;
  Slot createNewSlot();
  Slot getFreeSlot();
  Slot getSlot();
  SlotMetadata& getMetadataAt(ID id);
  const T& getAt(ID id) const;
  [[nodiscard]] void* get(civ::ID id) override;
...
struct Ref {
  ...
  const T& operator*() const { return (*array)[id]; }
...
struct PRef {
  ...
  T* operator->() { return provider_callback(id, provider); }

  T& operator*() { return *provider_callback(id, provider); }

  const T& operator*() const { return *provider_callback(id, provider); }
