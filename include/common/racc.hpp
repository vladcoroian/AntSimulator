...
struct RAccBase {
  ...
  uint32_t getCount() const { return std::min(current_index + 1, max_values_count); }
...
struct RMean : public RAccBase<T> {
  ...
  RMean(uint32_t max_size = 8) : RAccBase<T>(max_size), sum(0.0f) {}

  void addValue(T v) { sum += v - float(RAccBase<T>::addValueBase(v)) * RAccBase<T>::pop_value; }

  T get() const override { return sum / float(RAccBase<T>::getCount()); }
};
...
struct RDiff : public RAccBase<T> {
  ...
  void addValue(T v) { RAccBase<T>::addValueBase(v); }
