...
struct Simulation {
  ...
  civ::Ref<Colony> createColony(float colony_x, float colony_y) {
    ...
    const civ::ID colony_id =
        colonies.emplace_back(colony_x, colony_y, Conf::ANTS_COUNT);
  ...
  void removeDeadAnts() {
    ...
    for (Colony& colony : colonies) {
      ...
      if (killed) {
        ...
        renderer.colonies[colony.id].cleanVAs(initial_size - killed,
                                              initial_size);
      }
    }
    // Remove them
  ...
  void createColonyMarkers(Colony& colony) {
    for (uint32_t i(0); i < 64; ++i) {
      ...
      world.addMarker(
          colony.base.position +
              0.9f * colony.base.radius * sf::Vector2f(cos(angle), sin(angle)),
