...
struct Ant {
  enum class Type { Worker, Soldier };
  ...
  [[nodiscard]] static float getMarkerIntensity(float coef, float count) {
  ...
  void terminate() { autonomy = max_autonomy + 1.0f; }

  [[nodiscard]] bool isDone() const { return autonomy >= max_autonomy; }

  [[nodiscard]] bool isDead() const { return phase == Mode::Dead; }
};
