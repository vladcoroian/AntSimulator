...
struct PopulationChart {
  ...
  void render(sf::RenderTarget& target) {
    ...
    text.setPosition((position.x + population.width + padding) - bounds.width,
                     position.y + 0.5f * padding);
...
struct ColonyRenderer {
  ...
  explicit ColonyRenderer(civ::Ref<Colony> colony)
  ...
  void updatePopulation(float dt) { population.updateData(*colony_ref, dt); }
  ...
  void render_charts(sf::RenderTarget& target) { population.render(target); }
};
