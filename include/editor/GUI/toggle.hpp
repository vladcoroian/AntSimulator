...
struct Toggle : public Button {
  ...
  Toggle() : Button("", nullptr) {
    click_callback = [this]() { setState(!state); };
  ...
  void onPositionChange() override { updateTogglePosition(true); }
...
struct NamedToggle : public Container {
  ...
  explicit NamedToggle(const std::string& name)
    ...
    watch(toggle, [this]() { notifyChanged(); });
  }

  [[nodiscard]] bool getState() const { return toggle->state; }

  void setState(bool state) const { toggle->setState(state); }
