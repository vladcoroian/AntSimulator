...
struct ColorVariation : public GUI::Item {
  ...
  void onPositionChange() override { initializeVA(); }
...
struct HueSlider : public GUI::Item {
  ...
  void onSizeChange() override { updateVA(); }
  ...
  void onPositionChange() override { updateVA(); }

  void render(sf::RenderTarget& target) override { draw(target, hues_va); }
};
...
struct ColorPicker : public GUI::Container {
  ...
  explicit ColorPicker(sf::Vector2f size_ = {})
