...
struct ToolOption : public GUI::Button {
  ...
  void select() { select_padding = 5.0f; }

  void reset() { select_padding = 0.0f; }
  ...
  void render(sf::RenderTarget& target) override {
    ...
    GUI::RoundedRectangle color_rect(inner_size, position + inner_offset,
                                     tool_angle_radius - select_padding);
...
struct ToolSelector : public GUI::NamedContainer {
  enum class Tool { BrushWall, BrushFood, BrushDelete };
  ...
  explicit ToolSelector(ControlState& control_state_, Simulation& simulation_)
